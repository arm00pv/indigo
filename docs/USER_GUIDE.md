# Indigo MFA - User Guide

## Enrollment
To link your device to your organization:

1. **Mobile Users**:
   - Open the Indigo App.
   - Select "Setup via QR / Smart Code".
   - Scan the QR code provided by your IT team.

2. **Desktop Users**:
   - Open the Indigo Client (CLI).
   - Select "Setup via QR / Smart Code".
   - Copy the "Smart Code" (text) from the enrollment email or dashboard.
   - Paste it into the client.

## Using the App
- **Login**: Select "Login" to receive a push notification or decrypt a challenge.
- **Backup Codes**: If you lose your device, use one of the emergency backup codes saved during registration.
